...
class TemporalModule(nn.Module):
    ...
    def set_alpha(self, alpha: float) -> None:
        """Manually override the gating value.

        Useful for schedulers that manage the warm-up externally. The provided
        ``alpha`` is expected to be in ``(0, 1)`` and is converted to logit space
        before being stored in :attr:`alpha_param`.
        """

        logit = self._inv_sigmoid(alpha)
        target = getattr(self, "alpha_param")
        tensor = torch.tensor(logit, dtype=target.dtype, device=target.device)
    ...
    def alpha(self) -> float:
        """Return the current alpha value as a Python float."""

        return float(self._get_alpha().detach().cpu().item())
